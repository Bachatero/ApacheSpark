...
def simpleTokenize(string):
    ...
    """
    stringout = string.lower()
    
    #string = filter(lambda x:x != "", string)
    stringout = re.split(split_regex,stringout)
    stringout = filter(None, stringout)
...
def tokenize(string):
    ...
    #s1 = ' ,'.join(x for x in stringout if x not in stopwords)
    # list comprehension
    string1 = [x for x in stringout if x not in stopwords]
    #for word in stringout:
    #  if word in stopwords:
    #    stringout.remove(word) 
    #    print word 
    
    #stringout = filter(stopwords, stringout)
    return string1
    #return filter(lambda x: x == stopwords, simpleTokenize(string))
...
def tf(tokens):
    ...
    """
    tok_dict = {} # or dict = dict()
    tok_len = float(len(tokens))
    #print tok_len
    
    for word in tokens:
      #tok_dict[word] = float((tok_dict.get(word,0) + 1) / tok_len)
      tok_dict[word] = tok_dict.get(word,0) + 1 / tok_len
    return tok_dict

#quickbrownfox = 'A quick brown fox jumps over the lazy dog quick shits shits.'
...
def idfs(corpus):
    ...
    #print tokenCountPairTuple.take(2)
    tokenSumPairTuple = tokenCountPairTuple.reduceByKey(lambda a, b: a + b)
    return tokenSumPairTuple.map(lambda (x, y): (x,N/float(y)))
            
    #tokens = tokenSumPairTuple.filter(lambda x: x[1] > 1)
    #print tokens.take(1000)

idfsSmall = idfs(amazonRecToToken.union(googleRecToToken))
#print idfsSmall.take(5)
...
def tfidf(tokens, idfs):
    ...
    tfs = tf(tokens)
    #print type(tfs)
    #for keys, values in tfs.items():
    #    print keys, values
    #print tfs[0]
    #tfIdfDict = <FILL IN>
    tfIdfDict = {k: (tfs[k] * idfs[k]) for k in tfs}
    return tfIdfDict

recb000hkgj8k = amazonRecToToken.filter(lambda x: x[0] == 'b000hkgj8k').collect()[0][1]
#print recb000hkgj8k
#print type(recb000hkgj8k)
idfsSmallWeights = idfsSmall.collectAsMap()
#print type(idfsSmall)
#print type(idfsSmallWeights) 
#for keys, values in idfsSmallWeights.items():
#        print keys, values
...
def dotprod(a, b):
    ...
    """
    #dp =  {k: (a[k] * b[k]) for k in a }
    #return sum(dp.values())
    total = 0
    for key in a.viewkeys() & b.viewkeys():
        total = total + a[key] * b[key]
          #print a[key]
          #print b[key]
...
def norm(a):
    ...
    """
    no = {k: (a[k]**2) for k in a}
    #print no
    #print sum(no.values())**0.5
    return  sum(no.values())**0.5
...
#print similaritiesBroadcast.take(2)
...
#print trueDupsRDD.count()
#avgSimDups = trueDupsRDD.map(lambda (a,b): b).sum() / trueDupsCount
#print avgSimDups
#nonDupsRDD = (sims
#              .subtract(trueDupsRDD)
#             )
#print nonDupsRDD.count()
#avgSimNon = nonDupsRDD.map(lambda (a,b): b).sum() / float(39854)
#print avgSimNon
...
avgSimDups = trueDupsRDD.map(lambda (a,b): b).sum() / float(trueDupsCount)

nonDupsRDD = (sims
              .subtract(trueDupsRDD)
              #.map(lambda (a,(b,c)): (a,b))
             )
#print type(nonDupsRDD1) #.take(2)
nonDupsRDDCount = nonDupsRDD.count()

avgSimNon = nonDupsRDD.map(lambda (a,b): b).sum() / float(nonDupsRDDCount)
...
#print idfsFull.take(3)
#idfsFullCount = idfsFull.count()
#print 'There are %s unique tokens in the full datasets.' % idfsFullCount
#idfsFullWeights = idfsFull.collectAsMap()
#for key,val in idfsFullWeights.items():
#    print key,val
#print type(idfsFullWeights)

#print amazon.take(1)
#amazonWeightsRDD = amazon.sample(False, 0.001).map(lambda x: (x[0],tfidf(x[1],idfsFullBroadcast.value)))
#print amazonWeightsRDD.count()

#print google.take(1)
#print googleFullRecToToken.take(1)
#idfsFullBroadcast =  sc.broadcast(idfsFullWeights)
#googleWeightsRDD = googleFullRecToToken.sample(False, 1).map(lambda x: (x[0],tfidf(x[1],idfsFullBroadcast.value)))
#print googleWeightsRDD.count()
#print idfsFullWeights[0][0]
...
#print amazon.take(1)
#amazonWeightsRDD = amazon.map(lambda (k,v): (k,tfidf(v,idfsFullWeights)))
amazonWeightsRDD = amazonFullRecToToken.sample(False, 1).map(lambda x: (x[0],tfidf(x[1],idfsFullBroadcast.value)))
googleWeightsRDD = googleFullRecToToken.sample(False, 1).map(lambda x: (x[0],tfidf(x[1],idfsFullBroadcast.value)))
...
# TODO: Replace <FILL IN> with appropriate code
#RDD
amazonNorms = amazonWeightsRDD.map(lambda (a,(b)): (a,(norm(b))))
# convert RDD to dict
amazonNormsDict = amazonNorms.collectAsMap()
# convert dict to a broadcast variable
amazonNormsBroadcast = sc.broadcast(amazonNormsDict)

#print amazonNormsBroadcast
googleNorms = googleWeightsRDD.map(lambda (a,(b)): (a,(norm(b))))
googleNormsDict = googleNorms.collectAsMap()
googleNormsBroadcast = sc.broadcast(googleNormsDict)
...
#print invert(('basmeg', {'foo': 2, 'bar': 3}))
#print invert(('b000jz4hqo', {'rom': 2.4051362683438153, 'clickart': 56.65432098765432, '950': 254.94444444444443, 'image': 3.6948470209339774, 'premier': 9.27070707070707, '000': 6.218157181571815, 'dvd': 1.287598204264871, 'broderbund': 22.169082125603865, 'pack': 2.98180636777128}))

#print invertedPair 
#print invertedPair[0][1]
...
#commonTokens = (amazonInvPairsRDD.sample(False,0.01)
#                .join(googleInvPairsRDD.sample(False,0.01))
#                .map(lambda x: swap(x))
#                #.reduceByKey(add)
#                .groupByKey()
#                .cache())
...
def fastCosineSimilarity(record):
    ...
    tokens = record[1]
    #s = None
    #for t in tokens.viewkeys():
    #   s += dotprod(amazonWeightsBroadcast.value[amazonRec][t], googleWeightsBroadcast.value(googleRec)[t] )
